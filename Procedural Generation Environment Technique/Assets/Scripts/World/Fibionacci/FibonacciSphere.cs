...
public class FibonacciSphere
{
    public static Vector3[] GeneratePoints(int n, float radius, float jitter = 0)
    {
        ...
        //Generate amount fibionacci sphere points.
        for (int i = 0; i < n; i++)
        {
            Vector3 point = FibionacciSpherePoint(i, n, radius, jitter);
            points[i] = point;
        }
    ...
    public static HashSet<MyVector3> GeneratePointSet(int n, float radius, float jitter = 0)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            Vector3 p = FibionacciSpherePoint(i, n, radius, jitter);
    ...
    private static Vector3 FibionacciSpherePoint(int i, int n, float radius, float jitter)
    {
        //MATHS
        var j = i + .5f;
